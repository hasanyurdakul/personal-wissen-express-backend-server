...
userRouter.post("/register", async (req, res) => {
  try {
    ...
    if (!username || !email || !password) {
      return res
        .status(404)
        .send({ status: false, message: "Cannot leave empty fields!" });
    }
...
userRouter.post("/login", async (req, res) => {
  try {
    ...
    if (!username || !password || username == "" || password == "") {
      return res
        .status(404)
        .send({ staus: false, message: "Cannot leave empty fields!" });
    }
...
userRouter.post("/reset-password/:token", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .send({ status: false, message: "Invalid Token or Expired Token" });
    }
